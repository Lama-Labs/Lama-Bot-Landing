    ...
    if (eventType === "subscription.updated" || eventType === "subscription.created") {
        try {
            ...
            if (subscriptionData.items && Array.isArray(subscriptionData.items)) {
                const activePaidItems = subscriptionData.items.filter((item: any) =>
                    item.status === "active" &&
                    item.plan &&
                    item.plan.amount > 0
                );
                ...
                if (activePaidItems.length > 0 && subscriptionData.payer?.user_id) {
                    ...
                    if (user) {
                        ...
                        const existingApiKey = user.publicMetadata?.apiKey as string | undefined;
                        const existingVectorStoreId = user.privateMetadata?.vectorStoreId as string | undefined;
                        ...
                        if (!existingApiKey) {
                            ...
                            console.log(`Subscription updated for user: ${userId} (API key already exists)`);
                        }
                        ...
                        if (!existingVectorStoreId) {
                            ...
                            try {
                                ...
                                console.error(`Error creating vector store for user ${userId}:`, vectorStoreError);
                                return new Response("Error occurred while creating vector store", { status: 500 });
                            }
                        }
                    }
                    else {
